...
課題のまとめ
|  方式  |  敷居<br>（用意するもの）  |　正しさ　|　モチベーション維持 |
| ---- | ---- | --- | --- |
|  RFA（既存ゲーム）  |  高い（ゲーム機）  |　正しい（センサによる動作認識、補正のアドバイス） | 高い（応援、ゲーム性）|
|  動画による講座  |  低い（スマホ）  |　低い（見るだけ）| 低い |
|　何も使わない  | 低い（なし）　|　低い | 低い |
|　Virtual Kintore<br>（提案サービス） | 高い（カメラによる動作認識、上級者との比較）| 高い（応援、非日常の体験）|
